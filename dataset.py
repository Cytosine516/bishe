    ...
    def __getitem__(self, idx):
        try:
            ...
            for i in range(len(indices)):
                ...
                # 合并所有的无效像素 (SCL + 亮度判定云 + 阴影)
                invalid = scl_dilated | cloud_dilated | shadow_dilated
                valid = (1.0 - invalid.astype(np.float32))

                # 保存用于 loss 的 valid_mask
                obs_masks.append(torch.from_numpy(valid).unsqueeze(0))
